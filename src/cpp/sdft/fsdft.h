...
public:
  ...
  {
    const F delta = sample - exchange(analysis.inputs[analysis.cursor], sample);
    ...
    {
      const F state = delta + analysis.resonator[i][0] * analysis.fiddles[i] - analysis.resonator[i][1];
