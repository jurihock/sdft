...
def main():
    ...
    assert np.allclose(y['cpp'], y['py'], atol=1e-7), np.abs(y['cpp'] - y['py']).flatten().max()

    # check dfts

    shapes = list(set([_.shape for _ in dfts.values()]))
    assert len(shapes) == 1, f'{shapes}'
    assert np.allclose(dfts['cpp'], dfts['py'], atol=1e-7), np.abs(dfts['cpp'] - dfts['py']).flatten().max()

    # plot spectrograms

    figure('stft').spectrogram(stftx, sr, hopsize, xlim=(0, 7.9), ylim=(500, 15e3), yscale='log').tight()
    figure('cpp').spectrogram(dfts['cpp'], sr, hopsize, xlim=(0, 7.9), ylim=(500, 15e3), yscale='log').tight()
